...
## 2.3.1–2.5.0 (2026-09-13) — DONE

Site had last synced at 2.3.0. Counts re-derived at HEAD and at v2.3.0 with the same command:

| | Site had | Now | How |
|---|---|---|---|
| Built-in skills | 26 | **27** (Mark Quarry) | `ls data/legendquest/legendquest/skill` |
| Skill effect types | 14 | **15** (`legendquest:track`) | `SkillEffectTypes.register(` in `LQEffects.java` |
| Retheme-able strings | 318 | **347** | `def("` in `Lang.java` - it was already 323 at v2.3.0, so this had drifted before |
| Wasteland / Cold Frontier skills | 61 / 55 | **62 / 56** (Tag It, Tracer Tag) | files under `packs/*/.../skill/` |
| Config settings | 25 | 25 | `.define` in `LQConfig.java` - unchanged |

- **Standards is required from 2.5.0.** Hub gained a "Needs SableCraft Standards" tag, a requirements
  callout (1.8.0+, same Minecraft version), and "Two jars" as the heading; the `/game-plugins/` card's
  version line says so. `/content/`'s "Standards is entirely optional" sentence and the hub's
  "inert without it" / "if Standards is installed" wording were removed.
- `/content/`: heading 15 effect types, `legendquest:track` row (`duration` 300000, `glow` true, `target`
  looking_at, from `README.md`). `/skill-packs/` count fixed to 15.
- `/commands/`: `/lq roll [dice]` rewritten - the old row said "roll your statline", which `/roll` never
  did (it was a d20 before 2.5.0 too) - and `/lq skill toggle <skill>` added (2.4.0).
- Hub feature grid: "Dice that read like the table" (2.5.0 `/roll`) and "Passives you can switch off"
  (2.4.0) cards.
- NeoForge column now `21.11+` / `26.1+` / `26.2+` - 2.3.1 widened the range to any build in the line.
- Not changed: `/settings/`, `/permissions/`, `/packs/` beyond the two skill counts. 2.4.1's fixes (HUD
  vanishing with pinned debug lines, NPC nameplates) are not described anywhere on the site, so nothing to correct.
- New sibling product: **LegendQuest StoryTeller** got a `/game-plugins/` card placed straight after
  this one (no hub pages yet).
