...
**The loop does not follow a branch switch.** `run/` and `runBuddy/` are
gitignored, so pointing a worktree at `mc26.1`/`mc26.2` leaves a 1.21.11
environment behind it, and a 26.x session needs four things changed:

1. **Its own mods folder.** Every companion jar in `run/mods` is 1.21.11 and
   refuses to load on 26.x, which stops the server dead.
2. **Its own world.** The dev world is generated by CityWorld's chunk
   generator, so a run without CityWorld dies on `Overworld settings missing`.
   Move it aside rather than deleting it.
3. **Java 25 for the buddy.** `TestClient.cmd` hardcodes CurseForge's JDK 21
   (`java-runtime-delta`); 26.x needs `java-runtime-epsilon`.
4. **`pauseOnLostFocus:false` in `runBuddy/options.txt`.** Without it an
   unattended buddy sits on `PauseScreen` the moment the window loses focus,
   renders no in-game frame, and **any HUD bug is invisible to you**. This is
   very likely why the 26.x client was ported to compile and never once looked
   at. A dev-only warning screen will also wait for a human click — clear it
   with `-Dneoforge.warnings.logofile.hide=true`.

To actually *see* a HUD change rather than infer it: focus the client window
(`SetForegroundWindow`), `SendKeys('{F2}')`, and read the PNG from
...
- **`/zm observe on` makes the test player damage-immune** (ZombieMod, which is
  in `run/mods`). Turn it on before any long observation run — a death clears
  buffs and moves the player, and it will do it in the middle of the thing you
  were watching.
...
- **`showDebugScreen()` is not "is F3 up".** It is also true whenever *any*
  debug line has been pinned to stay on screen from the F3+F6 options, so
  guarding a HUD on it costs the player their whole HUD for the session while
  two lines sit in a corner. `LQHud` keys on `showFpsCharts()` /
  `showNetworkCharts()` / `showProfilerChart()` instead — those are gated on the
  overlay genuinely being visible, and they are the only parts that reach the
  bottom of the screen where our bars live.
- **When a faithful replication passes, the premise is wrong, not the
  replication.** A HUD reported missing "on 26.x" survived four replications of
  the reporter's instance — exact mod set, exact NeoForge build, matching
  character — because a dev client has nothing pinned. The probe printed
